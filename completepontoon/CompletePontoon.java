...
//        System.exit(0);
    ...
    //function to display images of the players card hand
    public void setImage() {
        int y = 0;
        for (int i = 0; i < newPlayer.getPlayerCards().size(); i++) {
            int x = i;
            String cardhand = newPlayer.getPlayerCards().get(i);
            Image imageno = new Image("resources/" + cardhand + ".jpg", 90, 150, false, false);
            CardHand.add(new ImageView(imageno), x, y);
            x++;
            if (i == 3) {
                x = i - 3;
            }
